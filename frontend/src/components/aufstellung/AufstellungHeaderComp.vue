<template>
	<div>
		<div class="header">
			...
			<span v-bind:style="{ color: isCM ? '#E91E63' : 'var(--v-textColor-base)' }">
			...
			>
				...
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn fab small color="blue" @click="toggleCopy" v-on="on">
							<v-icon>input</v-icon>
						</v-btn>
					</template>
				...
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn fab small color="green" @click="reloadBlanko" v-on="on">
				...
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn v-if="aufstellung.has_cm" fab small color="pink" @click="changeCM" v-on="on">
				...
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn fab small color="red" @click="deleteBoss" v-on="on">
			...
			<v-btn v-if="copyActive" color="blue" class="dial" small fab>
...
<script lang="ts">
	...
	export default Vue.extend({
		...
		methods: {
			...
			reloadBlanko: async function(): Promise<void> {
				await _aufstellungen.reloadBlanko(this.aufstellung.id);
