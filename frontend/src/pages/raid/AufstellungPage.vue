<template>
	<div>
		<TerminToolbarComp @extraRolesChanged="extraRolesChanged" />
		<v-progress-circular v-if="!aufstellungen" indeterminate color="primary" />
		<div v-if="aufstellungen">
			<v-container grid-list-md>
				<v-layout row wrap>
					<v-flex v-for="aufstellung in aufstellungen" :key="aufstellung.id" xs12 md6 xl3>
						<AufstellungComp :aufstellung="aufstellung" :showExtraRoles="showExtraRoles" />
					</v-flex>
				</v-layout>
			</v-container>
		</div>
...
<script lang="ts">
	...
	export default Vue.extend({
		...
		methods: {
			...
			extraRolesChanged: function(value: boolean) {
				this.showExtraRoles = value;
			}
		},
		computed: {
